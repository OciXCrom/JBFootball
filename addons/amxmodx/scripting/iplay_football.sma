...
{
	if(!(get_user_flags(id) & FLAG_BALL))
	{
	...
	col = cs_get_user_team(id) == CS_TEAM_CT || get_user_flags(id) & FLAG_BALL ? "r" : "d" 
	col2 = cs_get_user_team(id) == CS_TEAM_CT || get_user_flags(id) & FLAG_BALL ? "w" : "d" 
	 
	iLen = formatex(szBuffer, sizeof szBuffer - 1, "\r[\y%s\r] \wFootball Main Menu^n^n", PLUGIN_PREFIX) 
...
{ 
	...
	iLen = formatex(szBuffer, sizeof szBuffer - 1, "\r[\y%s\r] \wBall Control Room^n^n", PLUGIN_PREFIX) 
...
{ 
	...
	iLen = formatex(szBuffer, sizeof szBuffer - 1, "\r[\y%s\r] \wNet Control Room^n^n", PLUGIN_PREFIX) 
	...
	iLen += formatex(szBuffer[iLen], (sizeof szBuffer - 1) - iLen, "\%s4. \%sMove Net^n^n^n^n^n", col, col2) 
	iLen += formatex(szBuffer[iLen], (sizeof szBuffer - 1) - iLen, "\r0. \yBack") 
...
{ 
	if(!id || get_user_flags(id) & FLAG_BALL)
	{ 
		if(is_valid_ent(gBall)) 
		{ 
...
{ 
	if((key == 2 || key == 3 || key == 4) && !(cs_get_user_team(id) == CS_TEAM_CT || get_user_flags(id) & FLAG_BALL)) { 
		ShowMainMenu(id) 
		return PLUGIN_HANDLED 
	} 
...
{ 
	if(key != 9 && !(cs_get_user_team(id) == CS_TEAM_CT || get_user_flags(id) & FLAG_BALL)) { 
		ShowBallMenu(id) 
		return PLUGIN_HANDLED 
	} 
